...
## Beet Code status after this pass
...
- Current post-change verification: the full macOS suite executes 621 tests with 1 skip and 0 failures; the focused remote integration suite executes 5 tests with 0 failures.

### Remaining product gaps

| Priority | Gap | Why it matters | Suggested next slice |
| --- | --- | --- | --- |
| P1 | Per-model capability overrides | Gateways often report incomplete metadata or expose models with nonstandard limits. | Persist context, output, vision, tools, reasoning, temperature, and streaming overrides per remote model; surface them in the provider card. |
| P1 | Task/workspace sidebar | History is not enough for parallel work, background tasks, or isolated workspaces. | Add pinned tasks, active/running state, last activity, workspace path, and a review-needed badge; keep the current hybrid header as the navigation shell. |
| P1 | Specialist/write-capable subagents | Read-only research is useful but cannot parallelize implementation and verification. | Add explicit subagent roles with inherited approval policy, isolated checkpoints, and review-before-apply. |
| P1 | Verification loop | Aider and Cline make testing a visible part of completion. | Add a “verify” phase that runs the project’s detected checks, streams results into the transcript, and offers targeted repair. |
| P2 | Declarative project policy | Settings-only configuration is difficult to share and reproduce. | Add a non-secret project config for model defaults, allowed tools, plan policy, context paths, and output style; keep credentials in Keychain. |
...
1. Finish the per-model capability editor and make the effective profile visible in the composer/model picker.
2. Add task/workspace status to the hybrid sidebar and a first review-needed flow.
3. Add verification as a first-class agent phase.
4. Add specialist/write-capable subagents with isolated checkpoints.
5. Add project policy, share/import, and reconnectable background sessions.

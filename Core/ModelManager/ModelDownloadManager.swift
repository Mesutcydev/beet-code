...
/// Orchestrates a whole-model snapshot download (many files, sequentially)
/// and publishes UI-ready progress. Owned by AppState; the UI drives it via
/// `start/pause/cancel` only — start doubles as resume (completed files are
/// skipped, partial files resume from their sidecar byte offset).
/// Finalization (registration into ModelStore) happens through
/// `onCompletion`, never through UI row callbacks, so it is idempotent even
/// when the Model Manager sheet is closed.
@MainActor
final class ModelDownloadManager: ObservableObject {
    ...
    private var activeDownloaders: [String: SmartFileDownloader] = [:]
    ...
    private func run(model: CatalogModel, into directory: URL, hub: any HubServing) async {
        ...
        do {
            ...
            for (file, partialBytes) in pending {
                ...
                let destination = directory.appendingPathComponent(file.path)
                let source = hub.resolveURL(repo: model.repo, path: file.path)
                        ...
                        downloader.start(
                            progress: { [weak self] done, _ in
                                Task { @MainActor [weak self] in
                                    ...
                                    // The downloader's `done` is relative to
                                    // the ACTUAL resumed offset (0 after an
                                    // ETag restart), so progress never
                                    // over-reports stale partial bytes.
                ...
                case .success:
                    completed += file.sizeBytes
                case .failure(let error) where error == .paused:

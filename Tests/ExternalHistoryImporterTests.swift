...
final class ExternalHistoryImporterTests: XCTestCase {
    ...
    func testClaudeSkipsNonTextBlocks() {
        ...
        XCTAssertEqual(conversation?.messages.first?.content, "real question")
    }
    ...
    func testCodexParsesRolloutAndDropsContextInjections() {
        ...
        {"timestamp":"2026-07-04T08:55:17.000Z","type":"response_item","payload":{"type":"function_call","name":"shell"}}
        """
        ...
        XCTAssertEqual(conversation?.messages.count, 2)
        XCTAssertEqual(conversation?.messages.first?.content, "bump the version to 1.7")
        XCTAssertEqual(conversation?.messages.last?.role, .assistant)
    }

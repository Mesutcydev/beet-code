...
/// All are OpenAI-compatible chat-completions APIs except Gemini, which uses
/// its native generateContent format.
enum LLMProvider: String, CaseIterable, Codable, Sendable, Identifiable {
    ...
    /// Base URL for OpenAI-compatible endpoints (nil for Gemini's native API).
    ...
    var supportsVision: Bool {
        ...
        case .deepSeek, .longCat, .alibaba, .alibabaTokenPlan: false
        }
    }
...
final class APIKeyStore: ObservableObject {
    ...
    var configuredProviders: Set<LLMProvider> {
        Set(LLMProvider.allCases.filter { key(for: $0) != nil })
    }
